...
public:
  ...
  Colored<TColor>	m_color;	// the color that we found with ray-tracing
  struct
  {
...
public:
  RenderStruct(): m_curZ(0), m_bestZ(1e+20) { }
};
...
public:
  ...
  virtual const Colored<TColor>&	get_mipmap_color(uint32_t mipmap, uint32_t index, Colored<TColor>& __color) const 	= 0;	// note this is not called intensively, thus it can be under-optimized for time & optimized for memory
...
static inline const Colored<typename IContainer<T, N, TColor>::TYPEColor>& color_of(const typename IContainer<T, N, TColor>::TYPEColor& o, Colored<typename IContainer<T, N, TColor>::TYPEColor>& __color)
{
  return __color = o;
}  

template<typename T, uint32_t N, typename TColor>
static inline const Colored<TColor>& color_of(const IContainer<T, N, TColor>* o, Colored<TColor>& __color)
{
  return o->get_mipmap_color(o->get_Log2_ChildrenEdgeCount(), 0, __color);
}
template<typename T, uint32_t N, typename TColor>
static inline const Colored<TColor>& color_of(const IContainer<T, N, TColor>& o, Colored<TColor>& __color)
{
...
{
public:
  static constexpr uint64_t pow(uint64_t x, uint32_t n)
  {
...
protected:
  ...
  std::vector< std::vector<uint64_t> > 	m_mipmap__;
  uint64_t**				m_mipmap; // avoid operator calls and any extra checking
  uint64_t				m_mipmap_bits[64]; // avoid operator calls and any extra checking
  
  //std::vector< std::vector<Colored<TColor> > >  m_mipmap_color;
  std::vector< std::unordered_map<uint32_t, Colored<TColor> > >  m_mipmap_color;

  TChild 			m_emptyChild;  
  uint32_t 			m_Log2_ChildrenEdgeCount;
  Colored<TColor>	 	m_color;	// m_mipmap_color[m_Log2_ChildrenEdgeCount][0]
  Colored<TColor>		m_black;
  bool				m_mipmap_color_only;
 
public:
  Container(uint32_t __Log2_ChildrenEdgeCount, bool __mipmap_color_only = false): m_emptyChild{}, m_Log2_ChildrenEdgeCount(__Log2_ChildrenEdgeCount), m_mipmap_color_only(__mipmap_color_only)
  {
    m_mipmap 		= new uint64_t*[m_Log2_ChildrenEdgeCount + 1];
    fill(m_mipmap, m_mipmap + m_Log2_ChildrenEdgeCount + 1, (uint64_t*)0);
  ...
  const uint32_t m_mipmap_color_start_index = 1;
  
  
  inline void init_mipmap()
  {
    if (false == m_mipmap_color_only)
    {
      m_mipmap__.resize(m_Log2_ChildrenEdgeCount + 1);
      for (int i = 1; i <= m_Log2_ChildrenEdgeCount ; i++)
      {
	m_mipmap__[i].resize( pow(1u << (m_Log2_ChildrenEdgeCount-i), N) / 64 + 1 /* no need to husle with wether this +1 is needed or not :) */ );
	fill(m_mipmap__[i].begin(), m_mipmap__[i].end(), 0);
	m_mipmap[i] = m_mipmap__[i].data();
      }
...
//     }
  ...
  inline void add_child(const TChild_2& child, const Vec<T1, N>& __childPos, const Vec<T2, N>& __myPos, const T& __childEdgeSize, const T& __myEdgeSize)
  {
	...
	m_children[get_child_index(pos, 0)].add_child(child, __childPos, (Vec<T,N>(__myPos) + pos * zoomFactor) + ( /*zoomFactor / 2.0 either add it to both __childPos & objPos or to none*/ - __myEdgeSize / 2.0), __childEdgeSize, /*__myEdgeSize*/ (T)zoomFactor );
	
// 	// now update the mipmap
 	Colored<TColor> color;
 	color = color_of<T, N, TColor>(child, color);
 	
 	uint32_t index;
 	for (int mipmap = 1; mipmap < m_Log2_ChildrenEdgeCount+1; mipmap++) 
 	{
	  index = get_child_index(pos, mipmap);
	  
	  if (false == m_mipmap_color_only)
	  {
	    m_mipmap[mipmap][ index / 64 ] |= m_mipmap_bits[index % 64];
	  }
	  else
	  { 
	    ...
	    // FIXME: this is just used as a boolean (if not using m_mipmap), colors mustbe recalculated later
  ...
  virtual uint8_t get_mipmap(uint32_t mipmap, uint32_t index) const
  {
    if (false == m_mipmap_color_only)
    {
      return (m_mipmap[mipmap][ index / 64 ] & m_mipmap_bits[index % 64]) ? 1 : 0;
    }
    auto i = m_mipmap_color[mipmap].find(index);
    return i != m_mipmap_color[mipmap].end() && i->second;
  }
  
  virtual const Colored<TColor>&	get_mipmap_color(uint32_t mipmap, uint32_t index, Colored<TColor>& __color) const	// note this is not called intensively, thus it can be under-optimized for time & optimized for memory
  {
    ...
    if (mipmap == 0)
    {
      ...
      if (m_children.end() != i)
	return color_of<T, N, TColor>(i->second, __color);
      return Colored<TColor>::m_empty;
    }
	...
	index >>= (m_Log2_ChildrenEdgeCount-mipmap);
      ...
      {
	auto i = m_children.find(index);
	if (m_children.end() != i && (c1 = color_of<T, N, TColor>(i->second, c1)))
	  __color += c1;
    ...
    auto i = m_mipmap_color[mipmap].find(index);
    if (i != m_mipmap_color[mipmap].end())
      return i->second;
    return Colored<TColor>::m_empty;
  }
public:
...
protected:
  virtual Colored<Color<double>, int64_t> recalc_mipmap_color(uint32_t mipmap, uint32_t __index)
  {
    ...
    if (mipmap == 0)
    {
      ...
      if (m_children.end() != i)
	c += color_of<T, N, TColor>(i->second, c1);
      return c;
    } 
    ...
    if (mipmap >= m_mipmap_color_start_index && c)
    {
...
public:
  ...
  virtual const Colored<TColor>&	get_mipmap_color(uint32_t mipmap, uint32_t index, Colored<TColor>& __color) const
  { 
    Colored<TColor> c;
    __color.SetEmpty();
    for_each(m_children.begin(), m_children.end(), [&__color, &c](const ChildInfo& i)
    { 
    // FIXME: take only the color that is inside this container, the following will take the
      // //cout << "11: m_color" << m_color << " + " << color_of(i.child) << " = " << (decltype(m_color)(m_color) += color_of(i.child)) << endl;
      __color += color_of(i.child, c);
    });
    return __color;     
  }
  ...
  virtual bool 		render_child(RenderStruct<T, N, TColor>& rs, uint32_t child_index, const Vec<T, N>& childCenterPos /* my center: world units */, T childEdgeSize /* my size: world units */) const
  {
    ...
    { 
      ...
      { 
	if ( render(rs, *(i->child), (i->coordsDiff * (childEdgeSize * 1.0 / i->originalMyEdgeSize) ) + childCenterPos, i->originalChildEdgeSize * (childEdgeSize * 1.0 / i->originalMyEdgeSize) ) )
	  return true;
      }
    }      
    return false;
...
void render_compute_bounding_box(RenderStruct<T, N, TColor>& rs) 
{
  ...
  for (int i = 0; i < N; i++)
  {
    ...
    if (z1 > z2)
    {
      if (rs.m_tmp_maxZ > z1) 
      {
	 //cout << "2-render_compute_bounding_box : maxZ " << rs.m_tmp_maxZ << " " << z1 << endl;	
	rs.m_tmp_maxZ = z1;
      }
      if (rs.m_tmp_minZ < z2) 
      {
	 //cout << "2-render_compute_bounding_box : minZ " << rs.m_tmp_minZ << " " << z2 << endl;	
	rs.m_tmp_minZ = z2;
      }
    }
    else
    {
      if (rs.m_tmp_maxZ > z2) 
      {
	 //cout << "2-render_compute_bounding_box : maxZ " << rs.m_tmp_maxZ << " " << z2 << endl;	
	rs.m_tmp_maxZ = z2;
      }
      if (rs.m_tmp_minZ < z1) 
      {
	 //cout << "2-render_compute_bounding_box : minZ " << rs.m_tmp_minZ << " " << z1 << endl;	
	rs.m_tmp_minZ = z1;
      }
    }
  }
  ...
  if (nFailures >= N)
  {
    //cout << //cout << "2-render_compute_bounding_box : ERROR " << endl;
    rs.m_tmp_maxZ = 1e+20;
    rs.m_tmp_minZ = 2e+20;
  }  
}
...
bool render(RenderStruct<T, N, TColor>& __rs, const IContainer<T, N, TColor>& __o, const Vec<T1, N>& __objectCenterPos /* world units */, T __objectEdgeSize /* world units */)
{
   ...
   {
     {
      ...
      rs_local.m_tmp_minZ = __rs.m_curZ /*/ zoomFactor*/;
      rs_local.m_tmp_maxZ = __rs.m_bestZ /*/ zoomFactor*/;
      rs_local.m_tmp_adjustedLOS = &LOS;
      rs_local.m_tmp_objectEdgeSize = __objectEdgeSize;
      rs_local.m_tmp_plane1.m_org *= 0;

      render_compute_bounding_box(rs_local);
   ...
   else
  {
    ...
    rs_local.m_tmp_adjustedLOS = &LOS;
    rs_local.m_solid_angle_radius.m_a = __rs.m_solid_angle_radius.m_a / zoomFactor; 	// the zoom is already included in the LOS vector
    rs_local.m_solid_angle_radius.m_b = __rs.m_solid_angle_radius.m_b / zoomFactor; 	// the zoom is already included in the LOS vector
     //cout << "rs_local.m_screenPoint=" << rs_local.m_screenPoint << ", rs_local.m_LOS" << rs_local.m_LOS << " : T=" << typeid(T).name() << endl;
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    rs_local.m_tmp_maxZ = __rs.m_bestZ /*/ zoomFactor*/;
    rs_local.m_tmp_adjustedLOS = &LOS;
    rs_local.m_tmp_objectEdgeSize = 1u << LOG2_CHILDREN_EDGE_COUNT;
    rs_local.m_tmp_plane1.m_org *= 0;
    
    render_compute_bounding_box(rs_local);
    
    T minZ = rs_local.m_tmp_minZ;
    T maxZ = rs_local.m_tmp_maxZ;
    if (minZ > maxZ)
    {
      return false;
    }
    if (0 == __o.get_mipmap(LOG2_CHILDREN_EDGE_COUNT, 0))
     ...
     //cout << endl << "screenPoint " << screenPoint << " LOS.m_dir "<< LOS.m_dir << " minZ " << minZ << " maxZ " << maxZ << endl;    
    ...
    register double LOS_size = ~(LOS.m_dir);
    
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    // boundingBox-maxZ( coord[i] ) = boundingBoxB[i] + coord[i] * boundingBoxA[i]
    Vec<double, N> boundingBoxA;
    Vec<double, N> boundingBoxB;
    for (int i = 0 ; i < N; i++)
    {
      if (LOS.m_dir[i] == 0)
      {
	// LOS is parallel to this dimension
	boundingBoxA[i] = 0;
	boundingBoxB[i] = 0; // a huge number to not interfere with the other calculations
      }
      else
      {      
	// pos = LOS.m_org + curZ * LOS.m_dir; 
	// ==>
	// pos[i] = 0 = LOS.m_org[i] + (Z1 = B[i] + 0 * A[i]) * LOS.m_dir[i];
	// pos[i] = 1 = LOS.m_org[i] + (Z2 = B[i] + 1 * A[i]) * LOS.m_dir[i];
	// ==> 
	// -1 * LOS.m_org[i] / LOS.m_dir[i] = B[i]
	// ==>
	// (1 - LOS.m_org[i]) / LOS.m_dir[i] - B[i] = A[i]
	
	boundingBoxB.m_data[i] = -1 * LOS.m_org.m_data[i]  / LOS.m_dir.m_data[i];
	boundingBoxA.m_data[i] = (1 - LOS.m_org.m_data[i]) / LOS.m_dir.m_data[i] - boundingBoxB.m_data[i];
      }
    }
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    )
    {
      ...
      solid_angle_radius = 0.5 * ( rs_local.m_solid_angle_radius.m_b + curZ * rs_local.m_solid_angle_radius.m_a );
      
      index = __o.get_child_index(pos, mipmap);
      
      
      //cout << "render() : mipmap=" << mipmap << " curZ=" << curZ << " pos " << pos << " LOS.m_org " << LOS.m_org << " LOS.m_dir " << LOS.m_dir << " zoomFactor " << zoomFactor << " maxZ " << maxZ << " solid=" << solid_angle_radius << endl;
	...
	else
	{
	  ...
	  if (solid_angle_radius >= (1u << mipmap))
	  {
	    ...
	    __rs.m_curZ  = curZ;
	    __rs.m_bestZ = curZ;
	    __rs.m_color = __o.get_mipmap_color(mipmap, index, __rs.m_color);
	    return true;
	  }	  
	  ...
	  {
	    index = __o.get_child_index(pos, --mipmap);
	    //cout << " -- " <<  pos << " mipmap " << mipmap << " index " << index << endl << std::flush;
	  }
	  //cout << " => " << mipmap << endl;

	  if (mipmap > 0 && solid_angle_radius >= (1u << mipmap) && 0 != __o.get_mipmap(mipmap, index) )
	  {
	    // the solid angle here is bigger than the radius of the current box
	    // take the color of this box, and exit
	    //cout << "Found MIPMAP[" << mipmap << "==>" << (1u << mipmap) << "==>" << solid_angle_radius << "] color at " << pos << " bestZ= " << curZ << " => " << Vec<double, N>(pos) * zoomFactor + __objectCenterPos - __objectEdgeSize/2  << endl;
	...
	// if (false)
	{
	  //rs_local.m_tmp_minZ = -1e20; // curZ
	  //rs_local.m_tmp_maxZ = 1e+20;
	  //rs_local.m_tmp_objectEdgeSize = 1u << mipmap;	
	  // rs_local.m_tmp_plane1.m_org.set_binary_and(pos, p_mipmap_masks[mipmap]);	// (rs_local.m_tmp_plane1.m_org = pos) &= p_mipmap_masks[mipmap];  ==> ( (pos >> mipmap) << mipmap )		
	...
	// else
	{
	  ...
	  {	  
	    ...
	    double x = (pos.m_data[i] & p_mipmap_masks[mipmap]);
	    
	    if (boundingBoxA[i] > 0)
	    {
	      z1 = boundingBoxB.m_data[i] +  x * boundingBoxA.m_data[i];
	      z2 = boundingBoxB.m_data[i] + (x + mipmap_size) * boundingBoxA.m_data[i];
	    }
	    else
	    {
	      z2 = boundingBoxB.m_data[i] +  x * boundingBoxA.m_data[i];
	      z1 = boundingBoxB.m_data[i] + (x + mipmap_size) * boundingBoxA.m_data[i];
	    }
	    
	    if (rs_local.m_tmp_minZ < z1) rs_local.m_tmp_minZ = z1;
	    if (rs_local.m_tmp_maxZ > z2) rs_local.m_tmp_maxZ = z2;
	  }
	}
	...
	if (rs_local.m_tmp_minZ > rs_local.m_tmp_maxZ)
	{
	  cout << "render() : ERROR : failed to compute bounding box : mipmap=" << mipmap << " curZ=" << curZ << " " << rs_local.m_tmp_minZ << " " << rs_local.m_tmp_maxZ << " pos " << pos << " LOS.m_org " << LOS.m_org << " LOS.m_dir " << LOS.m_dir << " zoomFactor " << zoomFactor << " maxZ " << maxZ << endl;
	  return false;
	}	  
	else
	{
	  ...
	  if (curZ <= rs_local.m_tmp_maxZ) // - 0.3 / LOS_size * (1u << mipmap))
	    curZ = rs_local.m_tmp_maxZ + 0.01 / LOS_size * (1u << mipmap) /* make sure that we will hit the next target */;
	  else
	  {
	    // serious performance issue
	    cout << "false == (curZ < rs_local.m_tmp_maxZ - 0.3 / LOS_size * (1u << mipmap)) ; curZ " << curZ << " rs_local.m_tmp_maxZ " << rs_local.m_tmp_maxZ << " ; 1.0 / LOS_size * (1u << mipmap) = " << 1.0 / LOS_size * (1u << mipmap) << endl;
	    curZ += 0.25 / LOS_size * (1u << mipmap) /* make sure that we will hit the next target, note that we migh miss an corner that we would have just barely touched */;
	  }
	  //cout << "skipping box, curZ " << tmp_oldZ << " ==> " << curZ + 0.01 << " [] " << rs_local.m_tmp_minZ << " " << rs_local.m_tmp_maxZ << " zoomFactor " << zoomFactor << " LOS.m_dir " << LOS.m_dir << endl;
	}
	  
      }

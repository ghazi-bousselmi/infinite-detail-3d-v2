...
public:
  ...
  ~Container()	
  {
    for_each(m_children.begin(), m_children.end(), [](Vec<TChild, 1u << LOG2_CHILDREN_EDGE_COUNT>*& v) { if (v) delete v; v = 0; });
  }
  ...
  {
    auto i0 = index / (1u << LOG2_CHILDREN_EDGE_COUNT);
    if (m_children.m_data[i0] == 0)
      return m_emptyChild;
    return m_children.m_data[i0]->m_data[index % (1u << LOG2_CHILDREN_EDGE_COUNT)];
  }
  inline void set_child(uint32_t index, const TChild& child)
  {
    auto i0 = index / (1u << LOG2_CHILDREN_EDGE_COUNT);
    if (m_children.m_data[i0] == 0)
      m_children.m_data[i0] = new Vec<TChild, 1u << LOG2_CHILDREN_EDGE_COUNT>;
    m_children.m_data[i0]->m_data[index % (1u << LOG2_CHILDREN_EDGE_COUNT)] = child;
  }

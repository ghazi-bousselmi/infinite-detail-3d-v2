...
protected:
  ...
  // child(x, y, z) = m_children[x + y << m_Log2_ChildrenEdgeCount + z << (Log2_ChildrenEdgeCount * 2)]
  //Vec<Vec<TChild, 1u << m_Log2_ChildrenEdgeCount>*, pow(1u << m_Log2_ChildrenEdgeCount, N-1)> m_children;
  ...
  std::vector< std::vector<uint8_t /*bool*/> > m_mipmap;
...
bool render(RenderStruct<T, N, TColor>& __rs, const IContainer<T, N, TColor>& __o, const Vec<T1, N>& __objectCenterPos /* world units */, T __objectEdgeSize /* world units */)
{
   ...
   else
  {
    ...
    rs_local.m_bestZ = __rs.m_bestZ / zoomFactor;
	...
	else
	{
	  ...
	  while (mipmap > 0 && 0 != __o.get_mipmap(mipmap, index))
	  {
	    index = __o.get_child_index(pos, --mipmap);
	  }
	  //cout << " => " << mipmap << endl;

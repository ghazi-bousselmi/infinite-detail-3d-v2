...
bool render(RenderStruct<T, N, TColor>& rs, const TObject& o, const Vec<T, N>& objectCenterPos /* world units */, uint32_t objectEdgeSize /* world units */)
{
  ...
  else
  {
	...
	if (
	      // 0 != o.m_children[index]
	      render(rs, o.m_children[index], Vec<T, N>(pos*zoomFactor + objectCenterPos) + ( - objectEdgeSize/2 + (objectEdgeSize >> TObject::LOG2_CHILDREN_EDGE_COUNT)/2 ), objectEdgeSize >> TObject::LOG2_CHILDREN_EDGE_COUNT)
	    ) 
	{
	  // we got it

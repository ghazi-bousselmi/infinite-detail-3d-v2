...
inline TColor color_of(const TColor& o)
{
  return o;
}
...
public:
  ...
  virtual TColor	get_mipmap_color(uint32_t mipmap, uint32_t index) const 	= 0;	// note this is not called intensively, thus it can be under-optimized for time & optimized for memory
...
template<typename TColor, typename T, uint32_t N>
inline TColor color_of(const IContainer<T, N, TColor>* o)
{
  return o->get_mipmap_color(o->get_Log2_ChildrenEdgeCount(), 0).set_a(255);
}
template<typename TColor, typename T, uint32_t N>
inline TColor color_of(const IContainer<T, N, TColor>& o)
{
...
protected:
  ...
  std::vector< std::vector<TColor> >  m_mipmap_color;

  TChild 	m_emptyChild;  
  uint32_t 	m_Log2_ChildrenEdgeCount;
  TColor m_color;	// m_mipmap_color[m_Log2_ChildrenEdgeCount][0]
...
public:
  ...
  inline void init_mipmap()
  {
    ...
    for (int i = 1; i <= m_Log2_ChildrenEdgeCount ; i++)
    {
      ...
      fill(m_mipmap_color[i].begin(), m_mipmap_color[i].end(), TColor{});
    }
  }
  ...
  inline void add_child(const TChild_2& child, const Vec<T1, N>& __childPos, const Vec<T2, N>& __myPos, const T& __childEdgeSize, const T& __myEdgeSize)
  {
	...
	TColor color = color_of<TColor>(child);
	uint32_t index;
  ...
  virtual TColor get_mipmap_color(uint32_t mipmap, uint32_t index) const	// note this is not called intensively, thus it can be under-optimized for time & optimized for memory
  {
    if (mipmap == 0)
      return m_color;
...
public:
  ...
  virtual TColor	get_mipmap_color(uint32_t mipmap, uint32_t index) const 	{ return m_color; }

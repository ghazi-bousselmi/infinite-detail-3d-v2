...
public:  
  ...
  template<typename T1>
  explicit Vec(const initializer_list<T1>& l)
  {
    //assert(l.size() > 1);
  ...
  template<typename T1>  
  Vec& operator = (const initializer_list<T1>& l)
  {
    if (l.size() > N)
      copy(l.begin(), l.begin()+N, begin());
    else
  ...
  inline T operator ~ () const	{	return (T)sqrt( inner_product(begin(), end(), begin(), (T)0 ));	}  
  inline T length()	 const	{	return operator ~();	 					}
  inline T norm2 ()	 const	{	return (T)inner_product(begin(), end(), begin(), (T)0 );	}  
  ...
  inline Vec& set_binary_and(const Vec<T1, N>& v1, T2 v2)
  {	
    transform(v1.begin(), v1.end(), begin(), [v2](const T1& t1)->T{ return t1 & v2; }); 
    return *this;
  }
};
...
template<typename T1, typename T2, uint32_t N>
Vec<T1, N> operator * (const T2& t2, const Vec<T1, N>& v1)	{	return (Vec<T1, N>(v1) *= t2); }

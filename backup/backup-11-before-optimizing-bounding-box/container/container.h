...
public:
  Container(uint32_t __Log2_ChildrenEdgeCount, bool __mipmap_color_only = false): m_emptyChild{}, m_Log2_ChildrenEdgeCount(__Log2_ChildrenEdgeCount), m_mipmap_color_only(__mipmap_color_only)
  {
    ...
    fill(m_mipmap, m_mipmap + m_Log2_ChildrenEdgeCount + 1, 0);
...
// FIXME: pass these parameters inside a struct  
template<typename T, uint32_t N, typename TColor>
void render_compute_bounding_box_verboze(RenderStruct<T, N, TColor>& rs) 
{
  // ASSUME: the lower-left-innerMost-(...) corner of the object is at the coods (0, 0, ...)
...
bool render(RenderStruct<T, N, TColor>& __rs, const IContainer<T, N, TColor>& __o, const Vec<T1, N>& __objectCenterPos /* world units */, T __objectEdgeSize /* world units */)
{
   ...
   else
  {
	...
	rs_local.m_tmp_plane1.m_org.set_binary_and(pos, p_mipmap_masks[mipmap]);	// (rs_local.m_tmp_plane1.m_org = pos) &= p_mipmap_masks[mipmap];

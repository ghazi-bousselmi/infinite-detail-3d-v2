...
protected:
  ...
  //Vec<Vec<TChild, 1u << LOG2_CHILDREN_EDGE_COUNT>*, pow(1u << Log2_ChildrenEdgeCount, N-1)> m_children;
...
public:
  ...
  // adding an atomic child, with no consideration for its size, it is supposed to occupy only the cell at [pos]
  template<typename TChild_2>
  inline void add_child(const TChild_2& child, const Vec<uint32_t, N>& pos)
  {
    // FIXME: could be optimized for 3D
  ...
  template<typename TChild_2>
  inline void add_child(const TChild_2& child, const Vec<uint32_t, N>& __childPos, const Vec<uint32_t, N>& __myPos, const uint32_t& __childEdgeSize, const uint32_t& myEdgeSize)
  {
    //assert(myEdgeSize >= (1u << Log2_ChildrenEdgeCount));
    
    Vec<uint32_t, N> childPos = ((__childPos - __myPos) + myEdgeSize / 2.0) / ( myEdgeSize * 1.0 / (1u << Log2_ChildrenEdgeCount) );
    int32_t childEdgeSize = __childEdgeSize / ( myEdgeSize * 1.0 / (1u << Log2_ChildrenEdgeCount) );
    Vec<int32_t, N> deltaPos;
    fill(deltaPos.begin(), deltaPos.end(), -childEdgeSize);
    
    while (deltaPos.m_data[N-1] <= childEdgeSize)
    {
      // unsigned, not an issue, will overflow to the max
      Vec<uint32_t, N> pos = childPos + deltaPos;
	...
	for (int mipmap = 1; mipmap < Log2_ChildrenEdgeCount+1; mipmap++) 
      ...
      // advance the loop      
      deltaPos.m_data[0]++;
      for (int i = 0; i < N-1 && deltaPos.m_data[i] > childEdgeSize; i++)
      {
	deltaPos.m_data[i] = -childEdgeSize;
	deltaPos.m_data[i+1]++;
      }
    }
  }
  ...
  inline const TChild& get_child(uint32_t index) const
  {
  ...
  template<typename TChild_2>
  inline void set_child(uint32_t index, const TChild_2& child)
  {
    m_children[index].add_child(child);
  }
};
...
public:
  ...
  inline void add_child(const Color& _c)	{ c.u32 = _c.c.u32; }
};
...
template<typename T,
	 ...
	 typename TColor			// must define an enumeration { LOG2_CHILDREN_EDGE_COUNT }
	 >
bool render(RenderStruct<T, N, TColor>& rs, const TColor& o, const Vec<T, N>& objectCenterPos /* world units */, uint32_t objectEdgeSize /* world units */)
{
  /*
  ...
  Line<T, N> LOS(screenPoint, rs.m_LOS);
  ...
  else
  {
    ...
    cout << "TColor : " << minZ << " " << maxZ << endl;
  }
  */
  
  // FIXME:
  // this is an optimization: the upper code slows down the rendering by 7 folds.
  // since we are here, we assume that our caller already made sure that the line of sight intersects us
...
template<typename T,
	 ...
	 typename TColor			// must define an enumeration { LOG2_CHILDREN_EDGE_COUNT }
	 >
bool render(RenderStruct<T, N, TColor>& rs, const TObject& o, const Vec<T, N>& objectCenterPos /* world units */, uint32_t objectEdgeSize /* world units */)
{
  ...
  else
  {
	...
	auto child = o.get_child(index);
	if (		0 != child
	      &&	render(rs, child, Vec<T, N>(pos*zoomFactor + objectCenterPos) + ( - objectEdgeSize/2 + (objectEdgeSize >> TObject::LOG2_CHILDREN_EDGE_COUNT)/2 ), objectEdgeSize >> TObject::LOG2_CHILDREN_EDGE_COUNT)		) 
	{
	  // we got it
	  //rs.m_color = child;
